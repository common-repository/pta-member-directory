...
function pta_display_directory($location='', $position='') {
	...
	$dataTables = false;
	if(in_array('pta-member-directory-datatables/pta-member-directory-datatables.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )) {
		$dataTables = 'yes' === get_option('pta_md_datatables_enabled', 'no');
	}
	...
	<table class="pta_directory_table" id="pta_directory_table">
        <thead>
            ...
            if($options['enable_location'] && '' == $location) {
                ...
                $return .= '<th class="desktop">'.esc_html($options['location_label']).'</th>';
                $cols++;
            }
            if($options['show_photo']) {
                $return .= '<th class="desktop">&nbsp;</th>';
                $cols++; // add one to our colspan
            }
    ...
    foreach ($categories as $slug) {
	    ...
	    $group_contact_link = '';
        if ( 0 == $count) {
            if ('' != $location || !$options['show_vacant_positions'] || $dataTables) {
        ...
        while ( $member_loop->have_posts() ) : $member_loop->the_post();
	        ...
	        if($dataTables) {
		        if($show_positions) {
			        $return .= '<td style="vertical-align: middle;"><span class="pta-position-label"><strong>'.esc_html($category).'</strong></span>';
			        $return .= apply_filters( 'pta_directory_position_cell_before_group_contact_link', '', $term );
			        $return .= ' <br/>'. $group_contact_link;
			        $return .= '</td>';
		        }
            ...
            // Allow other plugins to add content cell after email
	...
	if(!$show_positions && !empty($group_contact_link)) {
...
function pta_directory_contact_form($id='', $location='', $position='', $hide_select = false) {
	...
	$label_recipient = apply_filters( 'pta_md_output', __('Or select a different recipient:', 'pta-member-directory'), 'select_different_recipient' ); // default message
	if ('-1' == $id) {
	...
	if ( isset($_POST['contact_mode']) && 'submitted' == $_POST['contact_mode'] ) {
	    ...
	    foreach ( $_POST as $field => $value ) {
	    	if("message" === $field) {
			    $form_data[$field] = stripslashes(sanitize_textarea_field( $value ) ); // get rid of any bad stuff
		    } else {
...
/**
 * Enqueue the style sheet and add inline styles from admin settings
 */
...
/**
 * callback function for the member directory shortcode
 *
 * @param $atts array shortcode attributes
 *
 * @return string member directory
 */
function pta_member_directory_shortcode($atts=array()) {
...
/**
 * callback function for the contact form shortcode
 *
 * @param $atts array shortcode attributes
 *
 * @return string contact form
 */
function pta_member_contact_shortcode($atts=array()) {
	extract( shortcode_atts( array(
			'location' => '',
			'position'	=> '',
			'id' => '',
...
/**
 * Callback for the admin contact shortcode
 * @return string admin contact form
 */
